...
QAE () {
  ...
  do
    ...
    if [ -f "${_PATH}" ]; then
      ...
      lmqg-qae -d "lmqg/qa_squadshifts_synthetic_random" -n "${LM}.${QAG}.${NAME}.seed_${RANDOM_SEED}" --output-dir "qa_eval_output/random_sampling.${LM}.${QAG}/${RANDOM_SEED}.${NAME}" --down-sample-size-train 1000 --down-sample-size-valid 500 --ray-result-dir "ray_results/ray_${RANDOM_SEED}.${LM}.${QAG}${NAME}"
